...
export class Ant extends WorldObject {
  ...
  private timeSinceLastPheromon = 0;
  private timeSinceTaskSwap: number = 0;
  ...
  private moveTowardsHive(dt: number, run = false) {
  ...
  private handlePheromons(dt: number) {
    if (this.currentTask === "runHome") return;
    if ((this.timeSinceLastPheromon += dt) < settings.pheromon.spawnCooldown) return;
  ...
  private findClosest<K extends WorldObject>(objects: K[]): K {
    if (objects.length === 0) throw new Error("Array is Empty");
