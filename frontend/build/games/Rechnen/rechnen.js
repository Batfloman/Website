...
function addieren(zahl1, zahl2) {
    while (zahl1 < level * 1.5)
        zahl1 = Math.ceil(Math.random() * level * 5);
    while (zahl2 < level * 1.5)
        zahl2 = Math.ceil(Math.random() * level * 2.5);
    ausgeben(zahl1 + " + " + zahl2);
    return zahl1 + zahl2;
}
function subtrahieren(zahl1, zahl2) {
    while (zahl1 < level * 3.5)
        zahl1 = Math.ceil(Math.random() * level * 5);
    while (zahl2 < level * 1.5 || zahl2 >= (zahl1 * 0.75))
        zahl2 = Math.ceil(Math.random() * level * 3.5);
    ausgeben(zahl1 + " - " + zahl2);
    return zahl1 - zahl2;
}
