...
export default class Ball {
    ...
    update(dt) {
        super.update(dt);
        if (this.touches("bottom") || this.touches("top") || this.touches(Paddle))
            this.bounce();
        if (this.touches("left") || this.touches("right"))
            this.reset();
    }
    render() {
    ...
    touches(objClass) {
        if (typeof objClass == "string") {
            if (objClass == "bottom")
                return this.pos.y + Ball.radius >= this.canvas.height;
            else if (objClass == "top")
                return this.pos.y - Ball.radius <= 0;
            else if (objClass == "left")
                return this.pos.x - Ball.radius <= 0;
            else if (objClass == "right")
                return this.pos.x + Ball.radius >= this.canvas.width;
            return false;
        }
