...
exports.Util = {
    ...
    math: {
        random: {
            between(start, end, num_decimals = 0) {
                return exports.Util.math.round(Math.random() * (end - start) + start, num_decimals);
            },
            // Vorzeichen
            mathSign() {
        ...
        trigonomitry: {
            cos(degree) {
                ...
                return Math.cos(exports.Util.math.convert.DegToRad(degree));
            },
            arccos(num) {
                return exports.Util.math.convert.RadToDeg(Math.acos(num));
            },
        },
    ...
    shapes: {
        ...
        polygon: {
            area(polygon) {
                return Polygon2Helper_js_1.Polygon2Helper.findArea(polygon);
            },
        },
    },
    object: {
    ...
    position: {
        ...
        worldPos_to_staticPos(worldPos, camara) {
            ...
            const staticPos = new Vector2_js_1.Vector2(exports.Util.math.round(distance.x, 5), exports.Util.math.round(-distance.y, 5)).add(camaraOffset);
            return staticPos;
        },
        ...
        staticPos_to_worldPos(staticPos, camara) {
            ...
            const worldPos = camara.pos.add(new Vector2_js_1.Vector2(exports.Util.math.round(distance.x, 5), exports.Util.math.round(-distance.y, 5)));
            return worldPos;
        },
        ...
        convertPercentInValue(canvas, widthPercent, heightPercent) {
            return new Vector2_js_1.Vector2(exports.Util.math.convert.percent(widthPercent, canvas.width), exports.Util.math.convert.percent(heightPercent, canvas.height));
        },
    ...
    toVector(angle, lenght) {
        const rad = exports.Util.math.convert.DegToRad(angle);
        return new Vector2_js_1.Vector2(Math.sin(rad) * lenght, Math.cos(rad) * lenght);
    },
    findAngleLine(startPoint, endPoint) {
        const zeroDegreeVector = new Vector2_js_1.Vector2(0, 1);
        ...
        const angle = exports.Util.math.trigonomitry.arccos(dot / (mag1 * mag2));
    ...
    distance(point1, point2) {
        return exports.Util.calcHypothenuse(point1.x - point2.x, point1.y - point2.y);
    },
    /**
    ...
    closestPoint(mainPoint, points, exclude) {
        let closest = new Vector2_js_1.Vector2(Infinity, Infinity);
        let closestDistance = Infinity;
        points.forEach((point) => {
            if (mainPoint == point)
                return;
            if (exclude instanceof Vector2_js_1.Vector2 && point == exclude)
                return;
            if (exclude instanceof Array && exclude.includes(point))
                return;
            let distance = exports.Util.distance(mainPoint, point);
    ...
    moveDirection(start, direction, distance) {
        const rad = exports.Util.math.convert.DegToRad(direction);
        const moveX = Math.sin(rad) * distance;
        const moveY = Math.cos(rad) * distance;
        return new Vector2_js_1.Vector2(start.x + moveX, start.y + moveY);
    },
    /**
    ...
    rotateAroundCenter(center, point, angle) {
        const rad = exports.Util.math.convert.DegToRad(angle);
        return new Vector2_js_1.Vector2(Math.cos(rad) * (point.x - center.x) - Math.sin(rad) * (point.y - center.y) + center.x, Math.sin(rad) * (point.x - center.x) + Math.cos(rad) * (point.y - center.y) + center.y);
    },
};
