...
class Renderer {
    constructor(canvas, camara) {
        this.fillColor = Color_js_1.Color.none;
        this.strokeColor = Color_js_1.Color.get("black");
        this.lineWidth = 1;
        ...
        this.updateValues();
        Input_js_1.Input.newEventListener("wheel", this, () => (this.zoomingChanged = true));
    }
    ...
    calcPosOnScreen(worldPos) {
        return Util_js_1.Util.position.worldPos_to_staticPos(worldPos, this.camara);
    }
    //#endregion
    // ==========================================================================================
    ...
    connectPoints(points) {
        ...
        for (let i = 0; i < points.length; i++) {
            let last = Util_js_1.Util.array.getItem(positions, i - 1);
            let current = Util_js_1.Util.array.getItem(positions, i);
            this.ctx.beginPath();
    ...
    renderPolygon(worldPos, polygon, angle, renderPoints = true, renderOutline = true) {
        let translated = Polygon2Helper_js_1.Polygon2Helper.translatePoints(polygon.model, worldPos, angle);
        if (renderOutline)
    ...
    convertStaticPosInValue(pos) {
        return Util_js_1.Util.position.convertStaticPosInValue(pos, this.camara);
    }
    convertPercentInValue(widthPercent, heightPercent) {
        return Util_js_1.Util.position.convertPercentInValue(this.canvas, widthPercent, heightPercent);
    }
    convertWidthPercentInValue(percent) {
        return Util_js_1.Util.position.convertWidthPercentInValue(this.canvas, percent);
    }
    convertHeightPercentInValue(percent) {
        return Util_js_1.Util.position.convertHeightPercentInValue(this.canvas, percent);
    }
    //#endregion
    // ==========================================================================================
    ...
    renderStaticGrid(pos, xSize, ySize, cellXSize, cellYSize, scaleLineWidth = true) {
        ...
        if (!(pos instanceof Vector2_js_1.Vector2))
        ...
        const topLeft = new Vector2_js_1.Vector2(pos.x - w / 2, pos.y - h / 2);
        this.ctx.beginPath();
        // vertical
    ...
    renderStaicLine(start, end, scaleLineWidth = true) {
        this.updateValues(scaleLineWidth);
        if (!(start instanceof Vector2_js_1.Vector2))
            start = this.convertStaticPosInValue(start);
        if (!(end instanceof Vector2_js_1.Vector2))
    ...
    // ==========================================================================================
    // #region setter
    setStrokeColor(color = Color_js_1.Color.none) {
    ...
    setFillColor(color = Color_js_1.Color.none) {
